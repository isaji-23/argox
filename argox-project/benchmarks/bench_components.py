...
def test_pii_processor_short_input(benchmark, run_context):
...
@pytest.mark.benchmark(group="processors")
def test_pii_processor_medium_input(benchmark, run_context):
    """PII redaction on a medium string (~500 chars) with mixed PII."""
    processor = PiiRedactionProcessor()
    text = "Email a@b.com or call 555-1234. " * 15
    benchmark(lambda: asyncio.run(processor.process_input(text, run_context)))


@pytest.mark.benchmark(group="processors")
def test_pii_processor_long_input(benchmark, run_context):
    """PII redaction on a long string (~10k chars)."""
    processor = PiiRedactionProcessor()
    long_text = "My email is test@example.com. " * 333
    benchmark(lambda: asyncio.run(processor.process_input(long_text, run_context)))


@pytest.mark.benchmark(group="processors")
def test_pii_processor_clean_input(benchmark, run_context):
    ...
    benchmark(lambda: asyncio.run(processor.process_input(clean_text, run_context)))


@pytest.mark.benchmark(group="processors")
def test_pii_processor_tool_args(benchmark, run_context):
    ...
    benchmark(lambda: asyncio.run(processor.process_tool_args("send_email", args, run_context)))
